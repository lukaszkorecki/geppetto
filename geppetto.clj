  ...
  (if (proc/alive? process)
    ...
    (let [exit-code (:exit @process)]
      (when-not (zero? exit-code)
        (throw (ex-info (format "ERROR: process exited with code %s\n" exit-code) {:babashka/exit 13}))))))
...
(defn -main [args]
  (let [conf-path (fs/expand-home (str (first args)))
        _ (when-not (fs/exists? conf-path)
            (println "ERROR: config file %s does not exist" conf-path)
            (System/exit 12))
        {:keys [tasks _settings] :as _conf} (load-config (str conf-path))
